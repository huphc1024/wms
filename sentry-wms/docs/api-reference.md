...
| Customer portal (3PL) | `POST /api/portal/auth/{login,logout,change-password}`, `GET /api/portal/auth/me`, `GET /api/portal/{inventory,inbound,orders,invoices}`, `GET /api/portal/orders/<so_number>`, `GET /api/portal/invoices/<invoice_number>`, `POST /api/portal/orders` | portal cookie (`sentry_portal_auth`) + CSRF; staff credentials rejected | [customer API guide](customer-api.md), [portal OpenAPI](api/portal-openapi.yaml). Every route gated on a `customer_user_permissions` feature grant and filtered through `customer_scope_clause`, so another customer's record answers 404 exactly as a nonexistent one does. Locators (bin, zone) are never published. A customer-bound `X-WMS-Token` (`wms_tokens.customer_id`) applies the same tenancy to the inbound and snapshot surfaces above. |
